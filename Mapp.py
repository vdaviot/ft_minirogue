...
class   Map_generator():
    ...
    def print_map(self, player, monster_table, turn):
        ...
        self.win.addnstr(17, 0, player.state, len(player.state))
        ...
        if player.target != None:
            if not "died" in player.target.state:
                self.win.addnstr(16, 0, player.target.state, len(player.target.state))
        self.win.refresh()
